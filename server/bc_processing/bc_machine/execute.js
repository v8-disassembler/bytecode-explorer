const getRegex = require('../bc_types/regex.js');
	...
	switch (op) {
      ...
      break;
		...
		case 'CallProperty1': {
			this.acc = this.store[args[0]];
			if (this.acc !== undefined) {
				this.acc = this.acc.call(this.store[args[1]], this.store[args[2]]);
			}
			break;
		}

		// call method with 2 argument
		case 'CallProperty2': {
			this.acc = this.store[args[0]];
			if (this.acc !== undefined) {
				this.acc = this.acc.call(this.store[args[1]], this.store[args[2]], this.store[args[3]]);
			}
			break;
		}
      ...
      break;
		...
		case 'JumpIfUndefined':
			if ((this.test = this.acc === undefined)) {
				const dest = getDest(args[1]);
				this.runningIndex = this.keyedCodeIndices[dest];
			}
			break;
		...
		case 'JumpIfUndefinedConstant':
			if ((this.test = this.acc === undefined)) {
				const dest = this.keyedCodeIndices[this.runningIndex] + this.constantPool[args[0]];
				this.runningIndex = this.keyedCodeIndices[dest];
			}
			break;
		...
		case 'CreateRegExpLiteral': {
			const pattern = this.constantPool[args[0]];
			const flags = args[2];
			this.acc = getRegex(pattern, flags); 
			break;
		}
